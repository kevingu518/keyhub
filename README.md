...
- **暴力破解防護** — 連續輸錯 5 次鎖定 60 秒，狀態持久化儲存（重開 popup 不會重置）
- **剪貼簿自動清除** — 複製 Key 後 30 秒自動清空，popup 關閉時即時清除
...
| 密碼政策 | 至少 12 字元，需含大寫、小寫英文、數字及特殊字元 |
| 暴力破解防護 | 5 次失敗 → 鎖定 60 秒（狀態持久化，重開 popup 不可繞過） |
| Session 逾時 | 閒置 5 分鐘自動鎖定 |
| 剪貼簿 | 複製後 30 秒自動清除 + popup 關閉時即時清除 |
| 網路請求 | 零對外連線 |
